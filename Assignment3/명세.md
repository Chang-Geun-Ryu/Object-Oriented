...
- `attack()`: 공격 의도(`AttackIntent`) 개체를 합니다. `AttackIntent` 안에는 공격 위치, 피해치, 공격자 등 공격에 필요한 정보가 담겨 있어야 합니다.
- `onAttack()`: 유닛에 피해치(damange)를 적용합니다.
...
- 유닛은 공격 구역에 있는 적만 공격할 수 있습니다.
- 유닛은 공격 구역에 있는 적을 발견하면 반드시 공격해야 합니다. 이동 가능한 유닛은 공격 구역에 있는 적을 발견하지 못한 경우에만 이동할 수 있습니다.
...
- 다음은 해병의 교전규칙입니다. (우선순위 순)
  ...
  2. 북쪽(위쪽)에 유닛이 있다면 그 타일을 공격. 그렇지 않을 경우 시계 방향으로 검색하다 찾은 유닛의 타일을 공격
...
- 다음은 망령의 교전규칙입니다. (우선순위 순)
  ...
  2. 가장 약한 유닛이 있는 타일을 공격
  3. 북쪽에 유닛이 있다면 그 타일을 공격. 그렇지 않을 경우 시계 방향으로 검색하다 찾은 유닛의 타일을 공격
...
- 다음은 미사일 포탑의 교전규칙입니다. (우선순위 순)
  ...
  2. 북쪽에 유닛이 있다면 그 타일을 공격. 그렇지 않을 경우 시계 방향으로 검색하다 찾은 유닛의 타일을 공격
